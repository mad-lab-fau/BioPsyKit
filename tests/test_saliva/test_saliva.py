...
def saliva_multiindex_wrong_name():
    return pd.DataFrame(index=pd.MultiIndex.from_product([range(0, 5), range(0, 5)], names=['subject', 'id']),
...
def saliva_pruessner_2003():
    data = pd.DataFrame(index=pd.MultiIndex.from_product([range(1, 3), range(0, 5)], names=['subject', 'sample']),
                        columns=['cortisol', 'time'])
...
class TestSaliva:
    ...
    @pytest.mark.parametrize(
        'input_data', [saliva_multiindex_wrong_name(), saliva_no_multiindex(), saliva_none()]
    )
    ...
    @pytest.mark.parametrize(
        "remove_s0, expected", params_max_increase
    )
    ...
    @pytest.mark.parametrize(
        "remove_s0, expected", params_max_increase_percent
    )
    ...
    def test_auc(self, remove_s0, expected):
        ...
        assert_frame_equal(saliva.auc(saliva_no_time(), remove_s0=remove_s0, saliva_times=[-10, 0, 10, 20, 30]),
                           expected, check_dtype=False)

        # check that 'saliva_time' parameter overrides 'time' column => set 'time' to 0
        data = saliva_time()
        data['time'] = 0
        assert_frame_equal(saliva.auc(data, remove_s0=remove_s0, saliva_times=[-10, 0, 10, 20, 30]), expected,
                           check_dtype=False)

    @pytest.mark.parametrize(
        "remove_s0, expected", params_auc_mutiple_pre
    )
    def test_auc_multiple_pre(self, remove_s0, expected):
        # check with 'time' column
        assert_frame_equal(saliva.auc(saliva_time_multiple_pre(), remove_s0=remove_s0), expected, check_dtype=False)
        # check with 'saliva_time' parameter
        assert_frame_equal(
            saliva.auc(saliva_time_multiple_pre(), remove_s0=remove_s0, saliva_times=[-20, -10, 0, 10, 20]), expected,
            check_dtype=False)
    ...
    def test_auc_pruessner2003(self):
        ...
        *Psychoneuroendocrinology*, 28(7), 916–931. https://doi.org/10.1016/S0306-4530(02)00108-7
        ...
        with pytest.warns(UserWarning):
            ...
            assert_frame_equal(data_out[['cortisol_auc_g', 'cortisol_auc_i']], expected, check_dtype=False)
        ...
        expected = pd.DataFrame(
            {
                ...
                'cortisol_std': [0.0, 0.0, np.std([0, 1, 2, 3, 4], ddof=1), np.std([5, 4, 3, 2, 1], ddof=1),
                                 np.std([10, 2, 4, 4, 6], ddof=1), np.std([-6, 2, 4, 4, 6], ddof=1),
                                 np.std([12, -6, 4, 4, 6], ddof=1), np.nanstd([2, np.nan, 8, 4, 6], ddof=1)],
                'cortisol_skew': [ss.skew([0, 0, 0, 0, 0], bias=False), ss.skew([1, 1, 1, 1, 1], bias=False),
                                  ss.skew([0, 1, 2, 3, 4], bias=False), ss.skew([5, 4, 3, 2, 1], bias=False),
                                  ss.skew([10, 2, 4, 4, 6], bias=False), ss.skew([-6, 2, 4, 4, 6], bias=False),
                                  ss.skew([12, -6, 4, 4, 6], bias=False), 0.0],
                'cortisol_kurt': [ss.kurtosis([0, 0, 0, 0, 0], fisher=False, bias=False),
                                  ss.kurtosis([1, 1, 1, 1, 1], fisher=False, bias=False),
                                  ss.kurtosis([0, 1, 2, 3, 4], bias=False), ss.kurtosis([5, 4, 3, 2, 1], bias=False),
                                  ss.kurtosis([10, 2, 4, 4, 6], bias=False), ss.kurtosis([-6, 2, 4, 4, 6], bias=False),
