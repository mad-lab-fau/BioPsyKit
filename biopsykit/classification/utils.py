...
def prepare_df_sklearn(data: pd.DataFrame, print_summary: Optional[bool] = False) -> Tuple:
    ...
    if print_summary:
        print("Shape of X: {}; shape of y: {}; number of groups: {}, class prevalences: {}"
              .format(X.shape, y.shape, len(group_keys), np.unique(y, return_counts=True)[1]))

    return X, y, groups, group_keys


def split_train_test(X: np.ndarray, y: np.ndarray, train: np.ndarray, test: np.ndarray,
